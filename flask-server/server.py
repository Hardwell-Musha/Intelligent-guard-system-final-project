...
def process_video(video_source, processing_duration=None, start_time=None, input_width=None, input_height=None, frame_rate=None):
    ...
    while True:
        ...
        # Define font size for the main text
        font_size = 80

        # Create ImageFont object with specified font size
        font = ImageFont.truetype("arial.ttf", font_size)

        # Text for persons and tracked persons
        text = f"PERSONS: {num_person_boxes}, TRACKED PERSONS: {num_tracked_boxes}"
        draw.text(text_position, text, font=font, fill=(255, 255, 255))

        # Calculate the position for the alert text
        alert_text_position = (70, text_position[1] + font_size + 20)  # Adjust the vertical position as needed

        # Display an alert if triggered
        if alert_triggered:
            # Increase font size for alert text
            alert_font = ImageFont.truetype("arial.ttf", font_size + 20)
            alert_text = "WEAPON DETECTED!"
            draw.text(alert_text_position, alert_text, font=alert_font, fill=(300, 300, 255))
